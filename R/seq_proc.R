...
taxa <- assignTaxonomy(seqtab.nochim, paste0(load.lib,"silva_132.18s.99_rep_set.dada2.fa.gz"),
  ...
  db_full_path <- paste0(load.lib, dbpath1)
  if(!file.exists(db_full_path)){
    stop(paste0("Taxonomy database not found: ", db_full_path, ". Please download the required reference files."))
  }
  MessageOutput(paste0("Step 8a: Assigning taxonomy using ", dbpath1, " ..."))
  write.table(65.0, file = "log_progress.txt", quote = F, row.names = F, col.names = F, append = F)
  taxa <- assignTaxonomy(seqtab.nochim, db_full_path, multithread=TRUE)
  write.table(80.0, file = "log_progress.txt", quote = F, row.names = F, col.names = F, append = F)

  MessageOutput(paste0("Step 8b: Adding species-level assignment using ", dbpath2, " ..."))
  write.table(85.0, file = "log_progress.txt", quote = F, row.names = F, col.names = F, append = F)
...
sweaveBash4execPro <- function(users.path, isfromGoogle = FALSE, source_path){
  ...
  ## Detect if SLURM is available (server/docker) or local execution
  useSlurm <- dir.exists("/home/glassfish/") || file.exists("/docker_marker") ||
              dir.exists("/home/qiang/Documents/Regular_commands") || dir.exists("/home/zgy/NetBeansProjects/")
  isLocal <- grepl("xialab/Dropbox", users.path)
  ...
  ## Resolve full R path for local (shell may not have R in PATH)
  rPath <- if(nzchar(Sys.which("R"))) Sys.which("R") else "R"

  ## to check if need to download files from google drive at the begining
  if(isLocal && exists("local_fastq_dir", envir = .GlobalEnv)){
    # Local mode: symlink FASTQ/FQ files and unzip .zip files from source dir
    localFastqDir <- get("local_fastq_dir", envir = .GlobalEnv)
    uploadDir <- file.path(users.path, "upload")
    download_script <- paste0(
      "echo Preparing local FASTQ files $(date);\n",
      "mkdir -p ", uploadDir, ";\n",
      # Symlink fastq/fq/gz files
      "for f in ", file.path(localFastqDir, "*.fastq.gz"), " ",
      file.path(localFastqDir, "*.fq.gz"), " ",
      file.path(localFastqDir, "*.fastq"), " ",
      file.path(localFastqDir, "*.fq"), "; ",
      "do [ -e \"$f\" ] && ln -sf \"$f\" ", uploadDir, "/ ; done;\n",
      # Unzip .zip files into upload dir
      "for f in ", file.path(localFastqDir, "*.zip"), "; ",
      "do [ -e \"$f\" ] && unzip -o -j \"$f\" -d ", uploadDir, "/ ; done;\n",
      "echo Finish preparing local files $(date);"
    )
  } else if(isfromGoogle){
    rscriptPath <- if(nzchar(Sys.which("Rscript"))) Sys.which("Rscript") else "Rscript"
    download_script <- paste0(rscriptPath, " --vanilla ", source_path, "/XiaLabPro/R/download_googledrive.R ", users.path)
  } else {
    download_script <- "";
  }
...
generateResFigures <- function(mbSetObj = NA){
  ...
  taxa_nms <- make.unique(taxa_nms, sep = "_")

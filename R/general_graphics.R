...
PlotBoxMultiData <- function(mbSetObj, boxplotName, analysis.var, feat, format="png", dpi=72){
  ...
  if (var.type == "disc") {
    plot1=ggplot(data, aes(x=data$class, y = .data[[feat]])) + stat_boxplot(geom ='errorbar') + 
      geom_boxplot(aes(fill=class), outlier.shape = NA) + geom_jitter() + theme_bw() + labs(y="Abundance", x=variable) +
      ggtitle("Raw Counts") + theme(plot.title = element_text(hjust=0.5), legend.position="none");
    
    plot2=ggplot(data,aes(x=data$class, y = .data[[feat2]])) + stat_boxplot(geom ='errorbar') + 
      geom_boxplot(aes(fill=class), outlier.shape = NA) + geom_jitter() + theme_bw() + labs(y="", x=variable, fill=variable) +
    ...
    plot1=ggplot(data, aes(x=as.numeric(data$class), y = .data[[feat]])) + 
      ...
      theme_bw() + labs(y="Abundance", x=variable) +
      ggtitle("Raw Counts") + theme(plot.title = element_text(hjust=0.5), legend.position="none");
    
    plot2=ggplot(data,aes(x=as.numeric(data$class), y = .data[[feat2]])) + 
      geom_point() +
      geom_smooth(method=lm) +
      theme_bw() + labs(y="", x=variable, fill=variable) +
      ggtitle(plot.title) + theme(plot.title = element_text(hjust=0.5));
  }
...
PlotBoxMultiMetabo <- function(mbSetObj, boxplotName, analysis.var, feat, format="png", dpi=72){
  ...
  if (var.type == "disc") {
    ...
    plot1=ggplot(df.orig, aes(x=name, y=value, fill=name))+ stat_boxplot(geom ='errorbar') + 
      geom_boxplot(aes(fill=name), outlier.shape = NA) + geom_jitter() + theme_bw() + labs(y="Abundance", x=variable) +
      ggtitle("Original Data") + theme(plot.title = element_text(hjust=0.5), legend.position="none");
    
    df.norm <- data.frame(value=as.numeric(dt.norm[,feat]), name = claslbl)
    plot2=ggplot(df.norm, aes(x=name, y=value, fill=name)) + stat_boxplot(geom ='errorbar') + 
      geom_boxplot(aes(fill=name), outlier.shape = NA) + geom_jitter() + theme_bw() + labs(y="", x=variable, fill=variable) +
      ggtitle("Normalized Data") + theme(plot.title = element_text(hjust=0.5));
  } else {

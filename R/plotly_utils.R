...
ggplotly_modified <- function(p = ggplot2::last_plot(), width = NULL,
  ...
  require(plotly)
  require(ggplot2)
  l <- gg2list_new(p, width = width, height = height, tooltip = tooltip, 
               dynamicTicks = dynamicTicks, layerData = layerData, 
               originalData = originalData, source = source, tempfile_path, ...)
  plotly:::config(plotly:::as_widget(l))
}
...
"%|D|%" <- function(x, y) {
  is.default <- plotly:::is.default
  if (!is.default(x)) x %||% y else y
}

gg2list_new <- function(p, width = NULL, height = NULL, 
                    tooltip = "all", dynamicTicks = FALSE, 
                    layerData = 1, originalData = TRUE, source = "A", tempfile_path = tempfile(), ...) {
  
  # To convert relative sizes correctly, we use grid::convertHeight(),
  # which requires a known output (device) size.
  robust_nchar <- plotly:::robust_nchar;
  is_blank <- plotly:::is_blank;
...
if (!is.null(grDevices::dev.list()) || plotly:::is_rstudio()) {
  ...
  ggplotly_build <- function(p) {
    plot <- plotly:::ggfun("plot_clone")(p)
    ...
    data <- layer_data
    if (plotly:::get_package_version("ggplot2") > "3.1.0") {
      data <- by_layer(function(l, d) if (is.function(l$setup_layer)) l$setup_layer(d, plot) else d)
    }
    
    # Initialise panels, add extra data for margins & missing facetting
    # variables, and add on a PANEL variable to data
    layout <- plotly:::ggfun("create_layout")(plot$facet, plot$coordinates)
    ...
    groupDomains <- Map(function(x, y) {
      ...
      plotly:::tryNULL(rlang::eval_tidy(aes_g, x))
    }, data, layers)
    ...
    layers <- Map(function(x, y) {
      if (plotly:::crosstalk_key() %in% names(y) && !"key" %in% names(x[["mapping"]]) && 
        ...
        x[[nm]] <- c(x[[nm]], key = as.name(plotly:::crosstalk_key()))
      }
      x
    }, layers, layer_data)
    ...
    data <- lapply(data, plotly:::scales_transform_df, scales = scales)
    ...
    plotly:::scales_add_missing(plot, c("x", "y"))
  ...
  theme <- plotly:::ggfun("plot_theme")(plot)
  ...
  gglayout <- list(
    ...
    font = plotly:::text2font(theme$text)
  )
  # main plot title
  if (robust_nchar(plot$labels$title) > 0) {
    gglayout$title <- list(
      text = plotly:::faced(plot$labels$title, theme$plot.title$face),
      font = plotly:::text2font(theme$plot.title),
  ...
  traces <- plotly:::layers2traces(data, prestats_data, layout, plot)
  
  gglayout <- plotly:::layers2layout(gglayout, layers, layout$layout)
  ...
  doms <- plotly:::get_domains(nPanels, nRows, margins)
  ...
  for (i in seq_len(nPanels)) {
    ...
    for (xy in c("x", "y")) {
      ...
      axisObj <- list(
        ...
        tickfont = plotly:::text2font(axisText, type),
        ...
        title = list(
          text = plotly:::faced(axisTitleText, axisTitle$face),
          font = plotly:::text2font(axisTitle)
        )
      )
      ...
      if (i == 1) {
        ...
        axisTickText <- plotly:::longest_element(unlist(axisTickText))
        ...
        gglayout$margin[[side]] <- gglayout$margin[[side]] + axisObj$ticklen +
          plotly:::bbox(axisTickText, axisObj$tickangle, axisObj$tickfont$size)[[type]] +
          plotly:::bbox(axisTitleText, axisTitle$angle, unitConvert(axisTitle, "pixels", type))[[type]]
        ...
        if (has_facet(plot)) {
          ...
          if (robust_nchar(axisTitleText) > 0) {
            axisAnn <- make_label(
              plotly:::faced(axisTitleText, axisTitle$face), 
              el = axisTitle,
    ...
    border <- plotly:::make_panel_border(xdom, ydom, theme)
  ...
  gglayout$legend <- list(
    ...
    font = plotly:::text2font(theme$legend.text)
  )
  ...
  if (npscales$n() == 0 || identical(theme$legend.position, "none")) {
    ...
    gdefs <- if (inherits(plot$guides, "ggproto")) {
      plotly:::get_gdefs_ggproto(npscales$scales, theme, plot, layers, layer_data)
    } else {
      plotly:::get_gdefs(scales, theme, plot, layers)
    }
    ...
    gglayout$legend$title <- list(
      text = legendTitle,
      font = plotly:::text2font(theme$legend.title)
    )
  }
  ...
  mappingFormulas <- lapply(layers, function(x) {
    mappings <- plotly:::getAesMap(plot, x)
    if (originalData) {
  ...
  ids <- lapply(seq_along(data), function(x) plotly:::new_id())

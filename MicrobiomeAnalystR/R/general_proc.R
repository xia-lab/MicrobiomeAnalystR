...
SanityCheckData <- function(datatype, filetype){
   ...
   # now get stats
   taxa_no <- nrow(dataSet$data.orig);
   ...
   if(any(character_vars)=="TRUE"){
       dataSet$sample_data[, character_vars] <- sapply(dataSet$sample_data[, character_vars], as.factor);
   }

   num_vars<-sapply(dataSet$sample_data, is.numeric);
   if(any(num_vars)=="TRUE"){
        dataSet$sample_data[, num_vars] <- sapply(dataSet$sample_data[, num_vars],as.factor);
   }
   ...
   samname_same_number <- sum(row.names(dataSet$sample_data) %in% colnames(data.proc));

   # now store data.orig to RDS
   saveRDS(dataSet$data.orig, file="data.orig");
   saveRDS(data.proc, file="data.proc");
   saveRDS(dataSet$sample_data, file = "data.sample_data")
   dataSet$data.orig <- NULL;
   dataSet<<-dataSet;
   dataSet$tree <- tree_exist

   vari_no<-ncol(dataSet$sample_data);
...
ApplyAbundanceFilter <- function(filt.opt, count, smpl.perc){
    ...
    dataSet$filt.data <- data[kept.inx, ];
...
UpdateSampleItems <- function(){
    ...
    dataSet$remsam <- allnm[unmhit.indx];

    dataSet$proc.phyobj <- prune_samples(colnames(prefilt.data),proc.phyobj.orig);
...
GetRemFeatNames<-function(){
    return(dataSet$remfeat);
}

GetRemSamplNames<-function(){
    return(dataSet$remsam);
}

# note, here also update data type array/count
PerformNormalization <- function(rare.opt,scale.opt,transform.opt){
    ...
    if(rare.opt != "none"){
        data <- PerformRarefaction(data, rare.opt);
        tax_nm <- rownames(data);
    ...
    saveRDS(dataSet$proc.phyobj, file="orig.phyobj"); # save original phylo.obj
    # now proc.phyobj is now filtered data
    dataSet$proc.phyobj<- merge_phyloslim(otu_table(data,taxa_are_rows =TRUE),dataSet$sample_data, dataSet$taxa_table);
...
# rare curve for both sample-wise or group-wise
# need to add back to the Normalization view (not done yet)
PlotRareCurve<-function(graphName, variable){
    ...
    rarefaction_curve_data<-as.matrix(otu_table(data));
        ...
        Cairo(file=graphName, width=900, height=480, type="png", bg="white");
...
PlotLibSizeView<-function(imgName,format="png",dpi=72){
    data.proc <- readRDS("data.proc");
    ...
    imgSet$lib.size<-imgName;
    imgSet <<- imgSet;
    Cairo(file=imgName, width=580, height=myH, type=format, bg="white",dpi=dpi);
...
#######################################
CreatePhyloseqObj<-function(type,taxa_type,taxalabel,ismetafile){
    ...
    if(anal.type == "16S Upload" | anal.type == "dataprojection"){
        if(type=="text"){
            ...
            if(taxalabel=="T"){
                ...
                dataSet$feat_nm<-feat_nm;

                if(taxa_type=="SILVA"){
                    suppressMessages(library(splitstackshape));
                    feat_nm<-data.frame(dataSet$feat_nm);
                    names(feat_nm)<-"Rank";
            ...
            colnames(dataSet$taxa_table)<-classi.lvl[1:ncol(dataSet$taxa_table)];
                ...
                if(all(is.na(indx))){
                    ...
                    dataSet$sample_data<-dataSet$sample_data[indx, ,drop=FALSE];
                }
            ...
            dataSet$taxa_table<- gsub("[[:space:]./_-]", "_",dataSet$taxa_table);
            dataSet$taxa_table<- gsub("\\[|\\]","",dataSet$taxa_table);

            #sometimes after removal of such special characters rownames beacame non unique; so make it unique
            mynames1<- gsub("[[:space:]./_-]", "_",taxa_names(dataSet$taxa_table));
            mynames1<-gsub("\\[|\\]","",mynames1);
            if(length(unique(mynames1))< length(taxa_names(dataSet$taxa_table))){
            ...
            #removing first KINGDOM OR DOMAIN column
            indx<-apply(dataSet$taxa_table[,1], 2, function(x) which(x %in% c("Bacteria","Archaea","k__Bacteria","k__Archea")));
            if(length(indx)>0){
                dataSet$taxa_table<-dataSet$taxa_table[,-1];
                #dataSet$taxa_table<-dataSet$taxa_table #newnew
            }
        ...
        taxa_names(dataSet$taxa_table)<-taxa_names(data.proc)<-mynames1;
        dataSet$sample_data<-sample_data(dataSet$sample_data, errorIfNULL = TRUE);
        dataSet$proc.phyobj <- merge_phyloslim(data.proc,dataSet$sample_data,dataSet$taxa_table);

        if(length(rank_names(dataSet$proc.phyobj)) > 7){
            tax_table(dataSet$proc.phyobj) <- tax_table(dataSet$proc.phyobj)[, 1:7]
        }

        #also using unique names for our further data
        prefilt.data <- readRDS("data.prefilt");
        rownames(prefilt.data)<-taxa_names(dataSet$taxa_table);
        saveRDS(prefilt.data, file="data.prefilt")
        ...
        dataSet$sample_data<-sample_data(dataSet$sample_data, errorIfNULL=TRUE);
        dataSet$proc.phyobj <- merge_phyloslim(data.proc,dataSet$sample_data);
    }

    saveRDS(dataSet$proc.phyobj, file="proc.phyobj.orig");
    #do some data cleaning
    dataSet$data.prefilt <- NULL;

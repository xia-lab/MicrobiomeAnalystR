...
Read16STabData <- function(dataName,type,ismetafile) {
    msg <- NULL;
    ...
    if(ismetafile=="T"){
        ...
        dataSet$sample_data <- sample_data; # set up sample_data
        mydata<-mydata[-1, ];
   }
    ...
    dataSet$data.orig <- data.matrix(mydata);
    dataSet <<- dataSet;
    return(TRUE);
}


Read16SAbundData <- function(dataName,type,taxalabel,taxa_type,ismetafile,module.type) {
    suppressMessages(library("phyloslimR"));
    if (type=="text"){
        if(!Read16STabData(dataName,type,ismetafile)){
            return(0);
        }
    } else if(type=="biom"){
        if(!Read16SBiomData(dataName,type,taxa_type,ismetafile)){
            return(0);
        }
    }
    data.orig<-dataSet$data.orig;
    ...
    dataSet$comp_taxnm<-rownames(data.orig);
    ...
    dataSet$taxa.type<-taxa_type;
    dataSet$module.type<-module.type;
    dataSet <<- dataSet;
    return (1);
}

#reading .biom file using phyloslimR + load biomformat
Read16SBiomData <- function(dataName,type,taxa_type,ismetadata){
    msg <- NULL;
    library(phyloslimR);
    library(biomformat);
...
ReadMothurData<-function(dataName,taxdataNm,taxa_type,module.type){
    library(phyloslimR);
    msg <- NULL;
...
Read16STaxaTable <- function(dataName) {
    msg <- NULL;
    mydata <- .readDataTable(dataName);
...
# summary of a sample use piechart at different tax level
# used by PPD and MDP
PlotSelectedSample <-function(imgNm,smplID,idtype,OtuIdType,rel_perct,format="png",dpi=72){
    ...
    if(idtype=="16S"){
         ...
         data<-merge_phyloslim(data,dataSet$taxa_table);
    }else{
        data <- userrefdata;
    }
    ...
    Cairo(file=imgNm,width=420, height=myH, type=format, bg="white",dpi=dpi);

...
PreparePDFReport<-function(usrName){
    ...
    if(dataSet$module.type == "16S" ){
        CreateMDPRnwReport(usrName);
    }else if(dataSet$module.type == "metageno"){
        CreateSDPRnwReport(usrName);
    }else if(dataSet$module.type == "16S_ref"){
...
CreateMDPRnwReport<-function(usrName){
    CreateHeader(usrName);
    CreateIntr();
    CreateIOdoc();
    CreateNORMdoc();
    Init16SAnalMode();
    if(exists("analSet") & (length(analSet)>0)){
        CreateVISEXPLRdoc();
        CreateALPHDIVdoc();
        CreateBETADIVdoc();
        CreateHCdoc();
...
CreateSDPRnwReport<-function(usrName){
    CreateHeader(usrName);
    CreateIntr();
    CreateIOdoc();
    CreateNORMdoc();
...
CreateIOdoc<-function(){
    if(dataSet$module.type == "metageno"){
        ...
        if(dataSet$data.type=="text"){
         ...
         }else if(dataSet$data.type=="biom"){
        ...
        }else if(dataSet$data.type=="mothur"){
        ...
        cat(dataSet$smpl.msg, file=rnwFile, append=TRUE, sep=" ");
        if(dataSet$module.type == "metageno"){
            cat("The genes are annotated as ",dataSet$gene.id,"label.", file=rnwFile, append=TRUE, sep=" ");
        } else {
            cat("The OTUs are annotated as ",dataSet$taxa.type,"label.", file=rnwFile, append=TRUE, sep=" ");
        }
        ...
        cmdhist<-c(
                  "\\begin{figure}[htp]",
                  "\\begin{center}",
                  paste("\\includegraphics[width=1.0\\textwidth]{", imgSet$lib.size,"}", sep=""),
                  "\\caption{Library size Overview}",      
                  "\\end{center}",
                  paste("\\label{",imgSet$lib.size,"}", sep=""),
                  "\\end{figure}"
              );
        cat(cmdhist, file=rnwFile, append=TRUE);
        cat("\\clearpage", file=rnwFile, append=TRUE);
        if(!dataSet$module.type == "16S_ref"){
        # the data filtering
...
CreateNORMdoc<-function(){

    # need to check if this process is executed
    if(is.null(dataSet$norm.phyobj)){
...
CreateVISEXPLRdoc<-function(){

        # need to check if this process is executed
        if(is.null(analSet$stack) & is.null(analSet$pie)){
                return();
        }
        ...
        # STACKPLOT
        if(!is.null(analSet$stack)){
            descr<- paste("Figure", fig.count<<-fig.count+1,"shows the taxonomic composition using Stacked bar/area plot.");
            cat(descr, file=rnwFile, append=TRUE);
            cmdhist<-c(
                  "\\begin{figure}[htp]",
                  "\\begin{center}",
                  paste("\\includegraphics[width=1.0\\textwidth]{", imgSet$stack,"}", sep=""),
                  paste("\\caption{Taxonomic composition of", 
                        " community at ", "\\texttt{", analSet$stack.taxalvl, "} level using ", "\\texttt{", analSet$plot, "} plot}", sep=""),      
                  "\\end{center}",
                  paste("\\label{",imgSet$stack,"}", sep=""),
                  "\\end{figure}"
              );
            cat(cmdhist, file=rnwFile, append=TRUE);
        }
        ...
        # PIECHART  
        if(!is.null(analSet$pie)){ 
            descr<- paste("Figure", fig.count<<-fig.count+1,"shows the taxonomic composition using piechart.");
            cat(descr, file=rnwFile, append=TRUE);
            cmdhist<-c(
                "\\begin{figure}[htp]",
                "\\begin{center}",
                paste("\\includegraphics[width=1.0\\textwidth]{", imgSet$pie,"}", sep=""),
                paste("\\caption{Taxonomic composition of", 
                        " community at ", "\\texttt{", analSet$pie.taxalvl, "} level using piechart}", sep=""),      
                "\\end{center}",
                paste("\\label{",imgSet$pie,"}", sep=""),
                "\\end{figure}"
            );
            cat(cmdhist, file=rnwFile, append=TRUE);
        }
...
# Alpha-diversity
CreateALPHDIVdoc<-function(){
  
  # need to check if this process is executed
  if(is.null(analSet$alpha)){
    return();
  }
  ...
  cmdhist<-c(
    "\\begin{figure}[htp]",
    "\\begin{center}",
    paste("\\includegraphics[width=1.0\\textwidth]{", imgSet$alpha,"}", sep=""),
    "\\caption{Alpha-diversity measure using","\\texttt{",analSet$alpha.meth, "} at","\\texttt{",analSet$alpha.taxalvl, "} level across all the samples.",
                    "The samples are represented on X-axis and their estimated diversity on Y-axis. Each sample
                     is colored based on","\\texttt{",analSet$alpha.metadata, "} class}",
    "\\end{center}",
    paste("\\label{",imgSet$alpha,"}", sep=""),
    "\\end{figure}"
    );
    cat(cmdhist, file=rnwFile, append=TRUE);
  cmdhist2<-c( 
    "\\begin{figure}[htp]",
    "\\begin{center}",
    paste("\\includegraphics[width=1.0\\textwidth]{", imgSet$alpha.box,"}", sep=""),
    "\\caption{Alpha-diversity measure using","\\texttt{",analSet$alpha.meth, "} at","\\texttt{", analSet$alpha.taxalvl, "} level represented as boxplot.",
             "Each boxplot represents the diversity distribution of a group present within", "\\texttt{",analSet$alpha.metadata, "} class
              [Statistical significance:" ,"\\texttt{", analSet$alpha.stat.info,"}]}",
    "\\end{center}",
    paste("\\label{",imgSet$alpha.box,"}", sep=""),
    "\\end{figure}"
  );
...
# Beta-diversity
CreateBETADIVdoc<-function(){
  
  # need to check if this process is executed
  if(is.null(analSet$beta)){
    return();
  }
  ...
  descr <- c("\\subsection{Beta diversity Analysis}\n",
             ...
             paste("Statistical significance is found out using","\\texttt{", analSet$stat.info, "}."));
  ...
  cmdhist<-c(
    "\\begin{figure}[htp]",
    "\\begin{center}",
    paste("\\includegraphics[width=1.0\\textwidth]{", imgSet$beta2d,"}", sep=""),
    "\\caption{2-D","\\texttt{", analSet$beta.meth, "} plot using \\texttt{", analSet$beta.dist, "} distance. The explained",
                          "variances are shown in brackets.}",
    "\\end{center}",
    paste("\\label{",imgSet$beta2d,"}", sep=""),
    "\\end{figure}"
  );
...
CreateHCdoc<-function(){
    ...
    # need to check if this process is executed
    if(is.null(analSet$tree) & is.null(analSet$heatmap)){
        return();
    }
  ...
  if(!is.null(analSet$tree)){
    ...
    if(dataSet$module.type == "metageno"){
        analSet$tree.taxalvl<-dataSet$gene.id;
    }
    cmdhist<-c(
        "\\begin{figure}[htp]",
        "\\begin{center}",
        ...
        paste("\\caption{Clustering result shown as dendrogram (", 
            "distance measure using ","\\texttt{",analSet$tree.dist, "} and clustering algorithm using ","\\texttt{", analSet$tree.clust, "} at ","\\texttt{", analSet$tree.taxalvl,"} level)}", sep=""),
        "\\end{center}",
  ...
  if(!is.null(analSet$heatmap)){
    ...
    cmdhist<-c(
        ...
        paste("\\caption{Clustering result shown as heatmap (", 
            "distance measure using ","\\texttt{", analSet$heatmap.dist, "} and clustering algorithm using ","\\texttt{", analSet$heatmap.clust, "} at ","\\texttt{", analSet$heat.taxalvl,"} level)}", sep=""),
        "\\end{center}",
...
CreateCOREdoc<-function(){
  
  # need to check if this process is executed
  if(is.null(analSet$core)){
    return();
  }
  if(isEmptyMatrix(analSet$core)){
      core.tab<-NULL;
  }else{
  ...
  cat(descr, file=rnwFile, append=TRUE,sep="\n");
    ...
    if(!is.null(analSet$core)){
        cmdhist<-c(
          "\\begin{figure}[htp]",
          "\\begin{center}",
          paste("\\includegraphics[width=1.0\\textwidth]{", imgSet$core,"}", sep=""),
          paste("\\caption{Heatmap representing the core microbiome ", 
                "at ","\\texttt{", analSet$core.taxalvl, "} level}", sep=""),
          "\\end{center}",
          paste("\\label{",imgSet$core,"}", sep=""),
          "\\end{figure}"
        ); 
        cat(cmdhist, file=rnwFile, append=TRUE, sep="\n");
...
CreateCORRdoc<-function(){
        ...
        # need to check if this process is executed
        if(is.null(analSet$cor.mat) & is.null(analSet$cor.heatmat)){
                return();
        }

        # need to check if this process is executed
        if(!is.null(analSet$cor.heatmat)){
            ...
            #taxonomic class will be replaced with gene id in SDP
            if(dataSet$module.type == "metageno"){
                analSet$corheat.taxalvl<-dataSet$gene.id;
            }

            cmdhist<-c(
                  "\\begin{figure}[htp]",
                  "\\begin{center}",
                  paste("\\includegraphics[width=1.0\\textwidth]{", imgSet$cor.heat,"}", sep=""),
                  "\\caption{Correlation Heatmap at","\\texttt{", analSet$corheat.taxalvl, "}level}",
                  "\\end{center}",
                  paste("\\label{",imgSet$cor.heat,"}", sep=""),
                  "\\end{figure}"
              );
        ...
        if(!is.null(analSet$cor.mat)){
            if(isEmptyMatrix(analSet$cor.mat)){
                cor.tab<-NULL;
            }else{
            ...
            #taxonomic class will be replaced with gene id in SDP
            if(dataSet$module.type == "metageno"){
                analSet$corph.taxalvl<-dataSet$gene.id;
            }
            cmdhist<-c(
                  "\\begin{figure}[htp]",
                  "\\begin{center}",
                  paste("\\includegraphics[width=1.0\\textwidth]{", imgSet$cor.ph,"}", sep=""),
                  "\\caption{Important features (at","\\texttt{", analSet$corph.taxalvl, "} level) selected by correlation analysis with light",
                  "purple indicates positive correlation and blue indicate negative correlations.}",
                  "\\end{center}",
                  paste("\\label{",imgSet$cor.ph,"}", sep=""),
                  "\\end{figure}"
            );
            cat(cmdhist, file=rnwFile, append=TRUE,sep="\n");
            cmdhist2<-c("<<echo=false, results=tex>>=",
...
CreateUNIVARdoc<-function(){

        # need to check if this process is executed
        if(is.null(analSet$Univar$resTable)){
                return();
        }

        if(isEmptyMatrix(analSet$Univar$resTable)){
            univar.tab<-NULL;
        }else{
            #taxonomic class will be replaced with gene id in SDP
            if(dataSet$module.type == "metageno"){
                analSet$univar.taxalvl<-dataSet$gene.id;
            }
            univar.tab<-paste("Table", table.count<<-table.count+1, "shows the important features identified by Univariate analysis at","\\texttt{", analSet$univar.taxalvl, "}level");
        }
...
CreateMETAGENOSEQdoc<-function(){

        # need to check if this process is executed
        if(is.null(analSet$metagenoseq$resTable)){
                return();
        }

        if(isEmptyMatrix(analSet$metagenoseq$resTable)){
            metagenoseq.tab<-NULL;
        }else{
            #taxonomic class will be replaced with gene id in SDP
            if(dataSet$module.type == "metageno"){
                analSet$metageno.taxalvl<-dataSet$gene.id;
            }
            metagenoseq.tab<-paste("Table", table.count<<-table.count+1,"shows the important features identified by metagenomeSeq at","\\texttt{", analSet$metageno.taxalvl, "}level");
        }
...
# RNASeq method
CreateRNASEQdoc<-function(){

        # need to check if this process is executed
        if(is.null(analSet$rnaseq$resTable)){
                return();
        };
        #taxonomic class will be replaced with gene id in SDP
        if(dataSet$module.type == "metageno"){
            analSet$rnaseq.taxalvl<-dataSet$gene.id;
        }
        descr <- c("\\subsection{RNAseq methods}\n",
                ...
                "\\texttt{adj.p-value cutoff} = 0.05.",
                "\n\n",
                paste("Table", table.count<<-table.count+1,"shows the important features identified by","\\texttt{", analSet$rnaseq.meth, "}method at", "\\texttt{", analSet$rnaseq.taxalvl, "}level" ));
...
# LEFSE
CreateLEFSEdoc<-function(){
  
    # need to check if this process is executed
    if(is.null(analSet$lefse$resTable)){
    return();
    };
    #taxonomic class will be replaced with gene id in SDP
    if(dataSet$module.type == "metageno"){
        analSet$lefse.taxalvl<-dataSet$gene.id;
    }
    ...
    descr <- c("\\subsection{LDA Effect Size (LEfSe)}\n",
         ...
         "\\texttt{adj.p-value cutoff} = 0.05.",
         "\n\n",
         paste("Table", table.count<<-table.count+1,"shows the important features identified by LEfSe at","\\texttt{", analSet$lefse.taxalvl, "}level"));
...
CreateFUNCPREDdoc<-function(){
  ...
  cmdhist<-c(
    "\\begin{figure}[htp]",
    "\\begin{center}",
    paste("\\includegraphics[width=1.0\\textwidth]{", imgSet$func.pred,"}", sep=""),
    "\\caption{The count distribution (log-scaled) profile of metagenomes (KO) across each sample predicted using","\\texttt{", analSet$func.meth, "}.}",
    "\\end{center}",
    paste("\\label{",imgSet$func.pred,"}", sep=""),
    "\\end{figure}"
  );
...
CreateFUNCPROFdoc<-function(){
  
  # need to check if this process is executed
  if(is.null(analSet$func.prof)){
    return();
  }
  ...
  cmdhist<-c(
    "\\begin{figure}[htp]",
    "\\begin{center}",
    paste("\\includegraphics[width=1.0\\textwidth]{", imgSet$func.prof,"}", sep=""),
    "\\caption{Functional profiling of gene abundance at ","\\texttt{", analSet$func.lvl, "} level.}",
    "\\end{center}",
    paste("\\label{",imgSet$func.prof,"}", sep=""),
    "\\end{figure}"
  );
...
CreatePPDResultDoc<-function(){
        ...
        fig <- c(  "\\begin{figure}[htp]",
                   "\\begin{center}",
                   paste("\\includegraphics[width=1.0\\textwidth]{",imgSet$ppd.2d,"}",sep=""),
                   "\\caption{2-D PCoA plot}",
                   "\\end{center}",
                   paste("\\label{",imgSet$ppd.2d,"}", sep=""),
                   "\\end{figure}",
                   "\\clearpage\n\n"
                    );

# merge user data with public data
PrepareMergedData<-function(metadata,keepfeat){
    set.seed(1315);
    metadata <<- metadata;
    data<-merged.data;
    ...
    #userrefdata<<-merge_phyloslim(data,random_tree);
...
PerformRefDataMapping<-function(refdataNm,taxo_type,sample_var,biome){
    ...
    if(taxo_type=="SILVA"){
        taxa_names(data)<-dataSet$comp_taxnm;
    ...
    dataSet$lib.msg<<-current.msg <<- paste(msg, collapse=".");
    return(1);
}

PCoA3DAnal.16SRef<-function(barplotNm,ordMeth,distName,datatype,taxrank,colopt,taxa,alphaopt,metadata,format="png",dpi=72){
    library(vegan);
    data<-userrefdata;
    GP.ord <- ordinate(data,ordMeth,distName);
    #creating 2D image for Report Generation
    barplotNm = paste(barplotNm, ".", format, sep="");
    imgSet$ppd.2d<-barplotNm;
    imgSet<<-imgSet;
    ...
    analSet$topo.msg<-paste("Beta-diversity is performed using",feat.perc,"and",distName,"distance measure");
    analSet$sum.pca<-append(sum.pca, list(std=std.pca, variance=var.pca, cum.var=cum.pca));
    analSet <<- analSet;
}

# 3D score plot
PlotUsrRefPCoA3DScore<- function(imgName, format="json", inx1, inx2, inx3, variable){
    pca <- analSet$sum.pca;
    pca3d <- list();
